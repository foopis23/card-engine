...
export function Card(props: CardProps) {
  ...
  const doFlip = useCallback(() => {
    if (userScript) {
      const hook: () => boolean | undefined | null =
        userScript.global.get("onFlip");
      if (hook) {
        const result = hook();
        if (result === false) {
          return;
  ...
  const doRelease = useCallback(() => {
    if (userScript) {
      const hook: () => boolean | undefined | null =
        userScript.global.get("onRelease");
      if (hook) {
        const result = hook();
        if (result === false) {
          return;
  ...
  const doDelete = useCallback(() => {
    if (userScript) {
      const hook: () => boolean | undefined | null =
        userScript.global.get("onDelete");
      if (hook) {
        const result = hook();
        if (result === false) {
          return;
  ...
  const rotateTowardsTargetRotation = useCallback(() => {
    ...
    const currentRot = new THREE.Euler().setFromQuaternion(
      new THREE.Quaternion(...Object.values(phyBody.rotation())),
    );
    ...
    if (owner.current === getUserId()) {
      if (selected.current) {
        ...
        if (raycaster.ray.intersectPlane(plane, intersectionPoint)) {
          targetPos.current = intersectionPoint.add(
            new THREE.Vector3(0, liftHeight, 0),
          );
        }
      }
    }
  ...
  useFixedTick(20, (_, delta) => {
    ...
    socket.emit(
      ...
      uuid,
      "position",
      Object.values(phyBody.translation()),
      Date.now(),
    );
