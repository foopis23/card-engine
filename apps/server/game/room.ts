...
export class Room implements Commands {
  ...
  setComponent(
    ...
    const entity = this.entities.get(entityId);
    if (!entity) {
      // this case will happen if one user deletes an entity that another user is interacting with. That fine, deletes are explict actions
      return;
    }
    ...
    if (!component) {
      // if the component doesn't exist, just set it
      entity.components.set(componentName, { value, lastWrite: timestamp });
      this.events.emit("componentSet", entityId, componentName, value);
    } else if (component.lastWrite < timestamp) {
      // if component does exist, use last write to determine if we should update
      entity.components.set(componentName, { value, lastWrite: timestamp });
      this.events.emit("componentSet", entityId, componentName, value);
    }
  }
