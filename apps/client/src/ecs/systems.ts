...
import { Flippable, Holdable, MainCamera, NetworkAuthority, Pointer, RigidBodyRef } from "./traits";
import { getUserId } from "../util/userId";
import { Euler, Plane, Quaternion, Raycaster, Vector3 } from "three";
import { positionFromVector3, quaternionFromRotation, rotationFromQuaternion, vector3FromPosition } from "../util/rapier";
		...
		if (flippable.flipped) {
			const target = new Quaternion()
				.setFromEuler(new Euler()
					.setFromVector3(new Vector3()
						.setFromEuler(new Euler()
							.setFromQuaternion(currentRot))
						.setZ(Math.PI)
					)
				);
	...
	world.query(
		...
		if (!holdable.held) {
			...
			if (raycaster.ray.intersectPlane(plane, intersectionPoint)) {
				...
				body.ref.setTranslation(positionFromVector3(
					currentPos.lerp(intersectionPoint.add(new Vector3(0, liftHeight, 0)), 0.4)
				), true);
			}
		}
	})
